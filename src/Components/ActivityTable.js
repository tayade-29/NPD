...
const ActivityTable = ({
  ...
  const saveActivity = () => {
    if (
      newActivity.phase &&
      newActivity.subActivities.some((sa) => sa.trim() !== "")
    ) {
      const updatedActivity = {
        phase: newActivity.phase,
        subActivities: newActivity.subActivities.filter(
          (sa) => sa.trim() !== ""
        ),
      };
  ...
  const handleSubActivityEdit = (activityIndex, subActivityIndex, value) => {
    setActivities((prev) => {
      ...
      updatedActivities[activityIndex].subActivities[subActivityIndex] =
        value;
    ...
    setActivities((prev) => {
      ...
      updatedActivities[activityIndex].subActivities =
        updatedActivities[activityIndex].subActivities.filter(
          (_, index) => index !== subActivityIndex
        );
      return updatedActivities;
    });
  };
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <select
                          ...
                          onChange={(e) =>
                            setActivities((prev) => {
                              const updated = [...prev];
                              updated[activityIndex].phase = e.target.value;
                              return updated;
                            })
                          }
                    ...
                    {activity.subActivities.map((subActivity, subIndex) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <button
                            onClick={() =>
                              deleteSubActivity(activityIndex, subIndex)
                            }
                            className="flex items-center text-red-600 hover:text-red-700"
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-gray-900">
                ...
                {isCheckpoints ? "Checkpoint" : "Activity"}
              </h3>
              <button
                onClick={() => {
                  setIsAdding(false);
            ...
            <div className="grid grid-cols-1 gap-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewActivity({ ...newActivity, phase: e.target.value })
                  }
                  ...
                  {newActivity.subActivities.map((subActivity, index) => (
                    <div key={index} className="flex gap-2">
                      <textarea
                        ...
                        value={subActivity}
                        onChange={(e) =>
                          handleSubActivityChange(index, e.target.value)
                        }
                        className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500 min-h-[80px]"
                      />
