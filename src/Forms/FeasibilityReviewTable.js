...
import {useGetEnquiriesFeasibilityQuery } from '../features/api/apiSliceenquiry'; // ✅ Update the path as needed

const TimePlanTable = () => {
  const navigate = useNavigate();

  const { data = [], isLoading, isError } = useGetEnquiriesFeasibilityQuery();

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold text-gray-800">Technical Feasibility Review and Team Commitment </h2>
      </div>
              ...
              data.map((enquiry, index) => (
                <tr key={index}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600">
                    ...
                    <button
  onClick={() => navigate('/feasibilityChart', { state: { enquiryId: enquiry.PkEnquiryMasterId } })}
